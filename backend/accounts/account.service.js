...
async function authenticate({ email, password, ipAddress }) {
    ...
    if (!account || !account.isVerified || !(await bcrypt.compare(password, account.passwordHash))) {
        throw messageProperties.LoginErrMsg;
    }
...
async function getUser(userId) {
    ...
    if (!user) throw messageProperties.UserNotFound;
    return user;
}
...
async function phoneDuplicateCheck(phoneNumber) {
    ...
    if(phoneObj.length>0){
        throw messageProperties.PhoneDuplicate;
    }
...
async function emailDuplicateCheck(email) {
    ...
    if(emailObj.length>0){
        throw messageProperties.EmailDuplicate;
    }
...
async function userNameDuplicateCheck(userName) {
    ...
    if(loginObj.length>0){
        throw messageProperties.UserNameAlreadyExist;
    }
...
async function getPhoneNumber(userId,phoneObject) {
    ...
    if (!phoneObj) throw messageProperties.PhoneNotfound;
    return phoneObj;
    }catch(err){
...
async function getEmail(userId,userEmail) {
    ...
    if (!emailObj) throw messageProperties.EmailNotFound;
    return emailObj;
    }catch(err){
...
async function _delete(useId) {
    ...
    if (!loginObj) throw messageProperties.UserNotFound;
    else{
        loginObj.UserNameVerified=2;
        await loginObj.save();
        return messageProperties.UserDeleteSuccesss;
